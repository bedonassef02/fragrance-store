...
<div class="bg-white/5 p-8 border border-white/10 sticky top-32">
    ...
    <div class="space-y-4 mb-8 text-sm text-gray-400">
        ...
        <div class="flex justify-between">
            <span>Shipping</span>
            <span class="text-white" id="cart-shipping">{{ $shipping > 0 ? number_format($shipping) . ' LE' : 'Free' }}</span>
        </div>
    ...
    @if($showCouponForm)
    <div class="mb-6 pt-4 border-t border-gray-800" id="coupon-section">
         @if($coupon)
            <div class="flex justify-between items-center bg-green-900/30 border border-green-800 p-3 rounded">
                ...
                <button type="button" id="remove-coupon-btn" class="text-gray-400 hover:text-white text-xs uppercase tracking-wider">Remove</button>
            </div>
        @else
            <form id="coupon-form" action="{{ route('cart.coupon.apply') }}" method="POST" class="flex gap-2">
                @csrf
                <input type="text" name="code" placeholder="Promo Code" class="flex-1 bg-black/50 border border-gray-700 px-4 py-3 text-sm text-white focus:border-moon-gold outline-none transition-colors" required>

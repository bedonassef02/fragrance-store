    ...
    <div class="pt-28 pb-20 bg-cream min-h-screen">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="text-center mb-12 pb-10 border-b border-neutral-200">
                <p class="text-accent uppercase tracking-[0.3em] text-xs font-medium mb-3">Brand Collection</p>
                <h1 class="text-4xl md:text-5xl font-serif text-charcoal mb-6">{{ $brand->name }}</h1>
                @if($brand->description)
                <div class="max-w-2xl mx-auto text-neutral-500 font-light leading-relaxed">
                    {{ $brand->description }}
                </div>
                @endif
            </div>
            ...
            <div class="flex flex-col lg:flex-row gap-12 relative">
                ...
                <aside id="shop-sidebar" class="fixed inset-0 z-50 lg:z-0 bg-cream p-6 overflow-y-auto transform -translate-x-full transition-transform duration-300 lg:relative lg:translate-x-0 lg:w-64 lg:inset-auto lg:p-0 lg:overflow-visible lg:block">
                    ...
                    <form action="{{ route('brands.show', $brand->slug) }}" method="GET" id="filter-form">
                        <div class="space-y-8">
                            <!-- Search -->
                            <div>
                                <h3 class="text-charcoal font-medium text-sm uppercase tracking-[0.1em] mb-4">Search</h3>
                                <div class="relative">
                                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Search in {{ $brand->name }}..." class="w-full bg-transparent border border-neutral-300 px-4 py-2.5 text-sm text-charcoal placeholder-neutral-400 focus:border-charcoal outline-none">
                        ...
                        <div class="mt-10 pt-6 border-t border-neutral-200">
                            ...
                            @if(request()->anyFilled(['search', 'category', 'concentration', 'capacity', 'price_range', 'sort', 'in_stock']))
                            <a href="{{ route('brands.show', $brand->slug) }}" class="block text-center mt-4 text-xs text-neutral-500 underline hover:text-charcoal transition-colors">Clear All Filters</a>
                            @endif
                        </div>
                    </form>
                </aside>
                ...
                <div class="flex-1">
                    <div class="grid grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-10">
                        ...
                        <div class="col-span-full text-center py-20">
                            ...
                            <p class="text-neutral-500">No products found for {{ $brand->name }} matching your filters.</p>
                            <a href="{{ route('brands.show', $brand->slug) }}" class="inline-block mt-4 text-charcoal underline text-sm">Clear filters</a>
                        </div>
                        @endforelse
                    </div>

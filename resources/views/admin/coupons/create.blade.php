    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
                ...
                <form action="{{ route('admin.coupons.store') }}" method="POST" class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="col-span-2">
                            ...
                            <div class="relative">
                                <input type="text" x-model="code" name="code" value="{{ old('code') }}" 
                        ...
                        <div>
                            ...
                            <input type="number" step="0.01" x-model="value" name="value" value="{{ old('value') }}" class="w-full bg-slate-900/50 border border-slate-700 rounded-xl p-3.5 text-white focus:ring-2 focus:ring-blue-500 font-mono" placeholder="20" required>
                        ...
                        <div>
                            ...
                            <input type="date" x-model="valid_from" name="valid_from" value="{{ old('valid_from') }}" class="w-full bg-slate-900/50 border border-slate-700 rounded-xl p-3 text-white focus:ring-2 focus:ring-blue-500">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-slate-400 mb-2">Valid To</label>
                            <input type="date" x-model="valid_to" name="valid_to" value="{{ old('valid_to') }}" class="w-full bg-slate-900/50 border border-slate-700 rounded-xl p-3 text-white focus:ring-2 focus:ring-blue-500">
                        </div>

                        <!-- Limits -->
                        <div>
                            <label class="block text-sm font-medium text-slate-400 mb-2">Usage Limit</label>
                            <input type="number" name="usage_limit" value="{{ old('usage_limit') }}" class="w-full bg-slate-900/50 border border-slate-700 rounded-xl p-3 text-white focus:ring-2 focus:ring-blue-500" placeholder="Unlimited">
                        </div>
                         <div>
                            <label class="block text-sm font-medium text-slate-400 mb-2">Min Order Amount</label>
                            <input type="number" step="0.01" name="min_order_amount" value="{{ old('min_order_amount') }}" class="w-full bg-slate-900/50 border border-slate-700 rounded-xl p-3 text-white focus:ring-2 focus:ring-blue-500" placeholder="0.00">
                        </div>
                        
                        <!-- Max Discount (Only for Percent) -->
                        <div x-show="type === 'percent'" x-transition class="col-span-2">
                            <label class="block text-sm font-medium text-slate-400 mb-2">Max Discount Amount (Cap)</label>
                            <input type="number" step="0.01" x-model="max_discount_amount" name="max_discount_amount" value="{{ old('max_discount_amount') }}" class="w-full bg-slate-900/50 border border-slate-700 rounded-xl p-3 text-white focus:ring-2 focus:ring-blue-500" placeholder="e.g. 100">
                             <p class="text-xs text-slate-500 mt-1">Maximum amount to discount (Capped at).</p>
                        </div>
                    </div>
        ...
        <div class="lg:col-span-1 sticky top-6">
            ...
            <p class="text-center text-slate-500 text-xs mt-4">
                This is how the coupon will appear in the admin panel.
            </p>
        </div>
    </div>
</div>

            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="border-b border-white/5 bg-white/5">
                        <th class="px-6 py-5 text-xs font-bold text-moon-gray-400 uppercase tracking-widest pl-8">Note Name</th>
                        <th class="px-6 py-5 text-xs font-bold text-moon-gray-400 uppercase tracking-widest">Type</th>
                        <th class="px-6 py-5 text-xs font-bold text-moon-gray-400 uppercase tracking-widest">Usage</th>
                        ...
                        <tr class="group hover:bg-white/[0.02] transition-colors duration-200">
                            <td class="px-6 py-4 pl-8">
                                <div class="flex items-center gap-4">
                                    <div class="w-10 h-10 rounded-full bg-white/10 flex items-center justify-center overflow-hidden border border-white/5 shrink-0 group-hover:border-moon-gold/30 transition-colors">
                                        @if($note->image)
                                            <img src="{{ $note->image }}" alt="{{ $note->name }}" class="w-full h-full object-cover">
                                        @else
                            ...
                            <td class="px-6 py-4 text-right pr-8">
                                <div class="flex items-center justify-end gap-2 opacity-60 group-hover:opacity-100 transition-opacity">
                                    ...
                                    <form action="{{ route('admin.notes.destroy', $note) }}" method="POST" onsubmit="return confirm('Delete this note?');" class="inline-block">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="p-2 text-moon-gray-400 hover:text-red-400 hover:bg-red-500/10 rounded-lg transition-all" title="Delete Note">
                        ...
                        <tr>
                            <td colspan="4" class="px-6 py-32 text-center">
                                <div class="flex flex-col items-center justify-center">
                                    ...
                                    <h3 class="text-2xl font-bold text-white mb-2 font-display">No Notes Found</h3>
                                    <p class="text-moon-gray-400 mb-8 max-w-sm mx-auto">Start building your fragrance library.</p>
                                    <a href="{{ route('admin.notes.create') }}" 

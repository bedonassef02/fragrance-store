...
<div class="flex items-center justify-between">
    <div>
        <h1 class="text-3xl font-bold text-white tracking-tight font-display">New Collection</h1>
        <p class="text-moon-gray-400 mt-1">Curate a new product collection.</p>
    </div>
</div>
@endsection

@section('content')
<form action="{{ route('admin.collections.store') }}" method="POST" enctype="multipart/form-data" class="max-w-3xl mx-auto">
    @csrf
            ...
            <div class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" name="title" id="title" value="{{ old('title') }}" required placeholder="e.g. Summer Essentials"
                ...
                <div>
                    ...
                    <textarea name="subtitle" id="subtitle" rows="2" placeholder="Brief description of this collection..."
                              class="w-full bg-black/20 border border-white/10 rounded-xl px-4 py-3 text-white placeholder-moon-gray-500 focus:border-moon-gold focus:ring-1 focus:ring-moon-gold transition-all resize-none">{{ old('subtitle') }}</textarea>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="layout_class" id="layout_class" value="{{ old('layout_class') }}" placeholder="e.g. col-span-2"
                               class="w-full bg-black/20 border border-white/10 rounded-xl px-4 py-3 text-white placeholder-moon-gray-500 focus:border-moon-gold focus:ring-1 focus:ring-moon-gold transition-all">
                    </div>
                    <div>
                        <label for="cta_text" class="block text-sm font-medium text-moon-gray-300 mb-2">CTA Text</label>
                        <input type="text" name="cta_text" id="cta_text" value="{{ old('cta_text') }}" placeholder="Shop Now"
                               class="w-full bg-black/20 border border-white/10 rounded-xl px-4 py-3 text-white placeholder-moon-gray-500 focus:border-moon-gold focus:ring-1 focus:ring-moon-gold transition-all">
                    </div>
                </div>
